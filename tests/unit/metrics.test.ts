...
import { createMetricsBuilder } from '../../src/metrics/builder.js';
import {
  METRIC_INPUT_COST,
  METRIC_OUTPUT_COST,
  METRIC_TOTAL_COST,
} from '../../src/metrics/definitions.js';
import type { CostResult } from '../../src/calculator/engine.js';
import type { MetricsBuilder } from '../../src/metrics/builder.js';
...
describe('createMetricsBuilder', () => {
  describe('instrument creation', () => {
    it('should create all three Counter instruments', async () => {
      ...
      const inputMetric = findMetric(metrics[0]!, METRIC_INPUT_COST);
      const outputMetric = findMetric(metrics[0]!, METRIC_OUTPUT_COST);
      const totalMetric = findMetric(metrics[0]!, METRIC_TOTAL_COST);
    ...
    it('should accept a prefix and prefix metric names', async () => {
      ...
      const prefixed = findMetric(metrics[0]!, `myapp.${METRIC_INPUT_COST}`);
