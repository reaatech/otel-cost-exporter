...
import type { PricingTableData } from './loader.js';
...
export function createPricingTable(initialData: PricingTableData): PricingTable {
  ...
  function rebuildReadonlyProviders(): {
    map: Map<string, ReadonlyMap<string, PriceEntry>>;
    count: number;
  } {
  ...
  const table: PricingTable = {
    ...
    async update(): Promise<void> {
      const { loadPricingData } = await import('./loader.js');
