...
program
  ...
  .action(async (options: { config?: string; port: string; metricsPort: string }) => {
    ...
    await serveCommand({
      ...
      metricsPort: Number.parseInt(options.metricsPort, 10),
    });
  });
...
program
  ...
  .action(async (options: { input?: string; format: string; environment?: string }) => {
    ...
    const { reportCommand } = await import('./commands/report.command.js');
...
program
  ...
  .action(async () => {
    const { validateCommand } = await import('./commands/validate.command.js');
